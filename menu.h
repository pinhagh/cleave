...
public:
	...
	Checkbox      extra_dmg;
	Slider        extra_dmg_amt;
	Checkbox	  penetrate;
	Slider		  penetrate_minimal_damage;
	Checkbox      knifebot;
	Checkbox	  zeusbot;
	Checkbox	  drawdebugaimbot;
...
public:
	void init() {
		...
		selection.setup(XOR("target selection"), XOR("selection"), { XOR("distance"), XOR("crosshair"), XOR("damage"), XOR("health"), XOR("latency"), XOR("height") });
		RegisterElement(&selection);

		fov.setup(XOR("fov"), XOR("fov"));
		RegisterElement(&fov);
		...
		multipoint.setup(XOR("multi-point hitboxes"), XOR("multipoint"), { XOR("head"), XOR("chest"), XOR("body"), XOR("legs") });
		RegisterElement(&multipoint);

		scale.setup("point scale", XOR("hitbox_scale"), 1.f, 100.f, true, 0, 90.f, 1.f, XOR(L"%"));
		scale.AddShowCallback(callbacks::IsMultipointOn);
		RegisterElement(&scale);

		body_scale.setup(XOR("body point scale"), XOR("body_hitbox_scale"), 1.f, 100.f, true, 0, 50.f, 1.f, XOR(L"%"));
		body_scale.AddShowCallback(callbacks::IsMultipointBodyOn);
		RegisterElement(&body_scale);

		hitbox_history.setup(XOR("backtrack hitboxes"), XOR("hitbox_history"), { XOR("head"), XOR("chest"), XOR("body"), XOR("arms"), XOR("legs"), XOR("feet") });
		...
		penetrate.setup(XOR("automatic penetration"), XOR("penetrate"));
		...
		minimal_damage_hp.setup(XOR("dynamic minimum damage"), XOR("minimal_damage_hp"));
		RegisterElement(&minimal_damage_hp);

		extra_dmg.setup(XOR("extra damage"), XOR("extra_dmg"));
		extra_dmg.AddShowCallback(callbacks::IsDynamicMinDmgOn);
		RegisterElement(&extra_dmg);

		extra_dmg_amt.setup("", XOR("extra_dmg_amt"), 1.f, 30.f, false, 0, 1.f, 1.f);
		extra_dmg_amt.AddShowCallback(callbacks::IsDynamicMinDmgOn);
		extra_dmg_amt.AddShowCallback(callbacks::IsExtraDmgOn);
		RegisterElement(&extra_dmg_amt);
		...
		drawdebugaimbot.setup(XOR("draw shot records"), XOR("drawdebugaimbot"));
		RegisterElement(&drawdebugaimbot);

		// col2.
		zoom.setup(XOR("automatic scope"), XOR("zoom"), { XOR("off"), XOR("always"), XOR("hitchance fail") });
		...
		lagfix.setup(XOR("fake-lag prediction"), XOR("lagfix"));
		RegisterElement(&lagfix, 1);

		correct.setup(XOR("anti-aim correction"), XOR("correct"));
		RegisterElement(&correct, 1);

		baim1.setup(XOR("prefer body aim"), XOR("baim1"), { XOR("always"), XOR("lethal"), XOR("fake"), XOR("in air") });
...
public:
	void init() {
		...
		mode.setup("", XOR("mode"), { XOR("stand"), XOR("walk"), XOR("air"), XOR("manual") }, false);
		...
		body_fake_stand.setup(XOR("fake body"), XOR("body_fake_stnd"), { XOR("off"), XOR("left"), XOR("right"), XOR("opposite"), XOR("twist"), XOR("z"), XOR("custom") });
		...
		manual_left.AddShowCallback(callbacks::IsAntiAimModeManual);
		RegisterElement(&manual_left);

		manual_right.setup(XOR("right"), XOR("manual_right"));
		manual_right.AddShowCallback(callbacks::IsAntiAimModeManual);
		RegisterElement(&manual_right);

		manual_backwards.setup(XOR("backwards"), XOR("manual_backwards"));
		manual_backwards.AddShowCallback(callbacks::IsAntiAimModeManual);
		RegisterElement(&manual_backwards);
		...
		lag_mode.setup("", XOR("lag_mode"), { XOR("max"), XOR("break"), XOR("break step"), XOR("fluctuate"), XOR("random")}, false);
		RegisterElement(&lag_mode, 1);

		lag_limit.setup(XOR("limit"), XOR("lag_limit"), 2, 16, true, 0, 2, 1.f);
		RegisterElement(&lag_limit, 1);

		lag_variance.setup(XOR("variance"), XOR("lag_limit"), 1.f, 100.f, true, 0, 50.f, 1.f, L"%");
		lag_variance.AddShowCallback(callbacks::IsFluctuateFakeLagOn);
		RegisterElement(&lag_variance, 1);

		lag_land.setup(XOR("lag on land"), XOR("lag_land"));
...
public:
	void init() {
		...
		box_enemy.setup(XOR("enemy color"), XOR("box_enemy"), { 255, 255, 255, 255 });
		RegisterElement(&box_enemy);

		box_friendly.setup(XOR("friendly color"), XOR("box_friendly"), { 255, 255, 255, 255 });
		...
		offscreen_color.setup(XOR("color"), XOR("offscreen_color"), { 255, 255, 255, 255 });
		RegisterElement(&offscreen_color);

		name.setup(XOR("name"), XOR("name"), { XOR("enemy"), XOR("friendly") });
		RegisterElement(&name);

		name_color.setup(XOR("color"), XOR("name_color"), { 255, 255, 255, 255 });
		...
		ammo_color.setup(XOR("color"), XOR("ammo_color"), { 95, 174, 227, 255 });
		RegisterElement(&ammo_color);

		lby_update.setup(XOR("lby update"), XOR("lby_update"));
		RegisterElement(&lby_update);

		lby_update_color.setup(XOR("color"), XOR("lby_update_color"), { 180, 60, 120, 255 });
		...
		chams_enemy_vis.setup(XOR("visible color"), XOR("chams_enemy_vis"), { 150, 200, 60 });
		RegisterElement(&chams_enemy_vis, 1);

		chams_enemy_invis.setup(XOR("invisible color"), XOR("chams_enemy_invis"), { 60, 180, 225 });
		...
		chams_friendly.setup(XOR("friendly chams"), XOR("chams_friendly"), { XOR("visible"), XOR("invisible") });
...
public:
	...
	Slider		  transparent_props_amt;
	Checkbox      enemy_radar;
	Checkbox	  aspect_ratio;
	Slider		  aspect_ratio_amt;
	Dropdown	  chams_enemy_material;
	Dropdown	  chams_enemy_history_material;
	Dropdown	  chams_friendly_material;
	Dropdown	  chams_local_material;
	...
	Checkbox      tracers;
	Colorpicker	  path_col;
	Checkbox      impact_beams;
	Colorpicker   impact_beams_color;
	Colorpicker   impact_beams_hurt_color;
	Slider        impact_beams_time;
	Keybind       thirdperson;
	Slider		  thirdperson_amt;

public:
	void init() {
		SetTitle(XOR("visuals"));

		chams_enemy_material.setup(XOR("enemy chams"), XOR("chams_enemy_material"), { XOR("default"), XOR("flat") });
		RegisterElement(&chams_enemy_material);

		chams_enemy_history_material.setup(XOR("enemy history chams"), XOR("chams_enemy_history_material"), { XOR("default"), XOR("flat") });
		RegisterElement(&chams_enemy_history_material);

		chams_friendly_material.setup(XOR("friendly chams"), XOR("chams_friendly_material"), { XOR("default"), XOR("flat") });
		RegisterElement(&chams_friendly_material);

		chams_local_material.setup(XOR("local chams"), XOR("chams_local_material"), { XOR("default"), XOR("flat") });
		RegisterElement(&chams_local_material);

		items.setup(XOR("weapons"), XOR("items"));
		RegisterElement(&items);

		ammo.setup(XOR("weapons ammo"), XOR("ammo"));
		RegisterElement(&ammo);
		...
		night_amt.setup("", XOR("nightmode_amt"), 1.f, 100.f, false, 0, 15.f, 1.f, XOR(L"%"));
		night_amt.SetCallback(Visuals::ModulateWorld);
		night_amt.AddShowCallback(callbacks::IsNightModeOn);
		...
		transparent_props_amt.setup("", XOR("transparent_props_amount"), 1.f, 100.f, false, 0, 50.f, 1.f, XOR(L"%"));
		transparent_props_amt.SetCallback(Visuals::ModulateWorld);
		transparent_props_amt.AddShowCallback(callbacks::IsPropTransparencyOn);
		RegisterElement(&transparent_props_amt);

		enemy_radar.setup(XOR("radar esp"), XOR("enemy_radar"));
		RegisterElement(&enemy_radar);

		aspect_ratio.setup(XOR("aspect ratio"), XOR("aspect_ratio"));
		aspect_ratio.SetCallback(Visuals::AspectRatio);
		RegisterElement(&aspect_ratio);

		aspect_ratio_amt.setup("", XOR("aspect_ratio_amt"), 1.f, 300.f, false, 0, 150.f, 1.f, XOR(L""));
		aspect_ratio_amt.SetCallback(Visuals::AspectRatio);
		aspect_ratio_amt.AddShowCallback(callbacks::IsAspectRatioOn);
		RegisterElement(&aspect_ratio_amt);

		// col2.
		removals.setup(XOR("removals"), XOR("removals"), { XOR("recoil"), XOR("smoke"), XOR("fog"), XOR("flashbang"), XOR("scope"), XOR("teammates")});
		RegisterElement(&removals, 1);

		fov.setup(XOR("custom fov"), XOR("fov"));
		RegisterElement(&fov, 1);

		fov_amt.setup("", XOR("fov_amt"), 60.f, 140.f, false, 0, 90.f, 1.f, XOR(L"°"));
		RegisterElement(&fov_amt, 1);

		fov_scoped.setup(XOR("while scoped"), XOR("fov_scoped"));
		fov_scoped.AddShowCallback(callbacks::IsCustomFOV);
		RegisterElement(&fov_scoped, 1);

		viewmodel_fov.setup(XOR("custom viewmodel fov"), XOR("viewmodel_fov"));
		...
		spread_xhair_col.setup(XOR("color"), XOR("spread_xhair_col"), colors::burgundy);
		...
		path_col.setup(XOR("color"), XOR("path_col"), { 255, 255, 255 });
		RegisterElement(&path_col, 1);

		impact_beams.setup(XOR("bullet tracers"), XOR("impact_beams"));
		RegisterElement(&impact_beams, 1);

		impact_beams_color.setup(XOR("normal color"), XOR("impact_beams_color"), colors::light_blue);
		RegisterElement(&impact_beams_color, 1);

		impact_beams_hurt_color.setup(XOR("hurt color"), XOR("impact_beams_hurt_color"), colors::red);
		RegisterElement(&impact_beams_hurt_color, 1);

		impact_beams_time.setup(XOR("time"), XOR("impact_beams_time"), 1.f, 10.f, true, 0, 1.f, 1.f, XOR(L"s"));
		...
		thirdperson_amt.setup("", XOR("thirdperson_amt"), 50.f, 300.f, false, 0, 150.f, 1.f, XOR(L"°"));
		RegisterElement(&thirdperson_amt, 1);
	}
};
...
public:
	void init() {
		...
		RegisterElement(&bhop, 1);

		airduck.setup(XOR("duck in air"), XOR("airduck"));
		RegisterElement(&airduck, 1);

		autostrafe.setup(XOR("automatic strafe"), XOR("autostrafe"));
		RegisterElement(&autostrafe, 1);

		unlock.setup(XOR("unlock inventory"), XOR("unlock_inventory"));
		...
		ragequit.setup(XOR("ragequit"));
		ragequit.SetCallback(callbacks::Ragequit);
		RegisterElement(&ragequit, 1);
	}
};
...
public:
	...
	void init() {
		...
		menu_color.setup(XOR("menu color"), XOR("menu_color"), { 255, 110, 75, 255 }, &g_gui.m_color);
