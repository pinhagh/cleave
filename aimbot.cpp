...
void AimPlayer::SetupHitboxes(LagRecord* record, bool history) {
	...
	// prefer head, running
	if (g_menu.main.aimbot.head1.get(1) && record->m_velocity.length_2d() > 71.f)
		m_hitboxes.push_back({ HITBOX_HEAD, HitscanMode::PREFER });

	// prefer head, resolved
	...
	// prefer, fake.
	if (g_menu.main.aimbot.baim1.get(2) && record->m_mode != Resolver::Modes::RESOLVE_NONE && record->m_mode != Resolver::Modes::RESOLVE_WALK && record->m_mode != Resolver::Modes::RESOLVE_BODY)
		m_hitboxes.push_back({ HITBOX_BODY, HitscanMode::PREFER });

	// prefer, in air.
	if (g_menu.main.aimbot.baim1.get(3) && !(record->m_pred_flags & FL_ONGROUND))
		m_hitboxes.push_back({ HITBOX_BODY, HitscanMode::PREFER });
...
bool AimPlayer::GetBestAimPosition(vec3_t& aim, float& damage, LagRecord* record) {
	...
	if (g_cl.m_weapon_id == ZEUS) {
		...
		if (g_menu.main.aimbot.minimal_damage_hp.get()) {
			...
			if (g_menu.main.aimbot.extra_dmg.get()) {
				dmg += g_menu.main.aimbot.extra_dmg_amt.get();
				pendmg += g_menu.main.aimbot.extra_dmg_amt.get();
			}
		}
	...
	if (attack || attack2) {
		...
		*g_cl.m_packet = false;
		...
		if (m_target) {
			...
			if (g_menu.main.aimbot.drawdebugaimbot.get())
