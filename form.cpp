...
void Form::draw() {
	...
	// colored line.
	render::line(m_x + 7, m_y + 7, m_x + m_width - 8, m_y + 7, { color });
	...
	if (!m_tabs.empty()) {
		...
		for (size_t i{}; i < m_tabs.size(); ++i) {
			...
			int pos_extra[7] = { 20, 20, 20, 23, 26, 26, 20 };
		...
		if (!m_active_tab->m_elements.empty()) {
			...
			std::string text = tfm::format(XOR("%s | %s | %s"), "cleave", "build 8/13/2021", g_cl.m_user);
