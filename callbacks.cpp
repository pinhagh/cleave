...
void callbacks::ConfigLoad1() {
	g_config.load(&g_menu.main, XOR("1.ac"));
...
void callbacks::ConfigLoad() {
	...
	std::string file = tfm::format(XOR("%s.ac"), config.data());
...
bool callbacks::IsPropTransparencyOn() {
	return g_menu.main.visuals.transparent_props.get();
}

bool callbacks::IsNightModeOn() {
...
bool callbacks::IsDynamicMinDmgOn() {
	return g_menu.main.aimbot.minimal_damage_hp.get();
}

bool callbacks::IsExtraDmgOn() {
	return g_menu.main.aimbot.extra_dmg.get();
}
...
bool callbacks::IsCustomLBY() {
	return g_menu.main.antiaim.body_fake_stand.get() == 6;
}

bool callbacks::IsCustomFOV() {
	return g_menu.main.visuals.fov.get() == 1;
}
...
bool callbacks::IsAntiAimModeManual() {
	return g_menu.main.antiaim.mode.get() == 3;
}
...
bool callbacks::IsFluctuateFakeLagOn() {
	return g_menu.main.antiaim.lag_mode.get() == 3;
}

bool callbacks::IsDrawDebugAimbotOn() {
	return g_menu.main.aimbot.drawdebugaimbot.get();
}

bool callbacks::IsAspectRatioOn() {
	return g_menu.main.visuals.aspect_ratio.get();
}
