...
std::string killsay = "Foste com o vento ;)";
...
void Shots::OnImpact(IGameEvent* evt) {
	...
	// this is a miss due to wrong angles.
		...
		else if (mode == Resolver::Modes::RESOLVE_UNKNOWN) {
			++data->m_unknown_move;
		}
...
void Shots::OnHurt(IGameEvent* evt) {
	...
	if (g_menu.main.misc.notifications.get(1)) {
		...
		if (hp == 0 && g_menu.main.misc.killsay.get()) {
			...
			switch (killsay) {
			case 0:
				g_csgo.m_engine->ExecuteClientCmd("say just skill and effort");
				break;

			case 1:
				g_csgo.m_engine->ExecuteClientCmd("say 1 retard");
				break;

			case 2:
				g_csgo.m_engine->ExecuteClientCmd("say foste com o vento");
				break;

			case 3:
				g_csgo.m_engine->ExecuteClientCmd("say have a nice day");
				break;

			case 4:
				g_csgo.m_engine->ExecuteClientCmd("say ouch");
				break;
			}
		}
	}
	...
	else if (mode == Resolver::Modes::RESOLVE_UNKNOWN && data->m_unknown_move > 0)
		--data->m_unknown_move;
