...
ulong_t __stdcall Client::init(void* arg) {
	...
	// welcome the user.
	g_notify.add("Welcome to cleave\n");
...
void Client::DrawHUD() {
	if (g_menu.main.misc.watermark.get()) {
		...
		std::string text = tfm::format(XOR("cleave | fps: %i | ms: %i"), fps, ms);
		...
		// background.
		render::rect_filled(m_width - size.m_width - 20, 10, size.m_width + 10, size.m_height + 2, { 38, 38, 38, 255 });
...
void Client::print(const std::string text, ...) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf(g_gui.m_color, XOR("[cleave] "));
...
void Client::Clantag() {
	auto SetClanTag = [&](std::string tag) -> void {
		...
		SetClanTagFn(tag.c_str(), XOR("cleave"));
	};
	...
	if (g_menu.main.misc.clantag.get()) {
		...
		if (iPrevFrame != int(g_csgo.m_globals->m_curtime * 2.9) % 14) {
			switch (int(g_csgo.m_globals->m_curtime * 2.9) % 20) {
			case 0: {  SetClanTag("<"); break; }
			case 1: {  SetClanTag("<"); break; }
			case 2: {  SetClanTag("c<"); break; }
			case 3: {  SetClanTag("cl<"); break; }
			case 4: {  SetClanTag("cle<"); break; }
			case 5: {  SetClanTag("clea<"); break; }
			case 6: {  SetClanTag("cleav<"); break; }
			case 7: {  SetClanTag("cleave<"); break; }
			case 8: {  SetClanTag("cleave<"); break; }
			case 9: {  SetClanTag("cleave<"); break; }
			case 10: { SetClanTag("cleave<"); break; }
			case 12: { SetClanTag("cleave<"); break; }
			case 13: { SetClanTag("cleav<"); break; }
			case 14: { SetClanTag("clea<"); break; }
			case 15: { SetClanTag("cle<"); break; }
			case 16: { SetClanTag("cl<"); break; }
			case 17: { SetClanTag("c<"); break; }
			case 18: { SetClanTag("<"); break; }
			case 19: { SetClanTag("<"); break; }
