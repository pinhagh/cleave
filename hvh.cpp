...
void HVH::DoRealAntiAim() {
	...
	if (m_yaw > 0) {
		...
		if (stand && !g_cl.m_lag && g_csgo.m_globals->m_curtime >= (g_cl.m_body_pred - g_cl.m_anim_frame) && g_csgo.m_globals->m_curtime < g_cl.m_body_pred) {
			// z mode.
			if (g_menu.main.antiaim.body_fake_stand.get() == 5)
		...
		// run normal aa code.
			...
			switch (m_yaw) {
				...
				// direction.
			case 1:
				if (g_input.GetKeyState(g_menu.main.antiaim.manual_left.get())) {
					left = true;
					right = false;
					backwards = false;
					g_cl.m_cmd->m_view_angles.y += -90.f;
				}
				else if (g_input.GetKeyState(g_menu.main.antiaim.manual_right.get())) {
					left = false;
					right = true;
					backwards = false;
					g_cl.m_cmd->m_view_angles.y += 90.f;
				}
				else if (g_input.GetKeyState(g_menu.main.antiaim.manual_backwards.get())) {
					left = false;
					right = false;
					backwards = true;
					g_cl.m_cmd->m_view_angles.y += 180.f;
