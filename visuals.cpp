...
void Visuals::ModulateWorld() {
	...
	if (g_menu.main.visuals.world.get() == 1) {
		float night_amt = g_menu.main.visuals.night_amt.get();

		for (const auto& w : world)
			w->ColorModulate(night_amt / 100, night_amt / 100, night_amt / 100);
	...
	if (g_menu.main.visuals.transparent_props.get()) {
		...
		for (const auto& p : props)
			p->AlphaModulate(g_menu.main.visuals.transparent_props_amt.get() / 100.f);
	}
...
void Visuals::ThirdpersonThink() {
	...
	if (g_csgo.m_input->CAM_IsThirdPerson()) {
		...
		offset.z = g_menu.main.visuals.thirdperson_amt.get();
...
void Visuals::Hitmarker() {
	...
	render::line(x - 4 - line, y - 4 - line, x - (line / 4) - 3, y - (line / 4) - 3, { 200, 200, 200, alpha });
	render::line(x - 4 - line, y + 4 + line, x - (line / 4) - 3, y + (line / 4) + 3, { 200, 200, 200, alpha });
	render::line(x + 4 + line, y + 4 + line, x + (line / 4) + 3, y + (line / 4) + 3, { 200, 200, 200, alpha });
	render::line(x + 4 + line, y - 4 - line, x + (line / 4) + 3, y - (line / 4) - 3, { 200, 200, 200, alpha });
}
...
void Visuals::StatusIndicators() {
	...
	for (size_t i{ }; i < indicators.size(); ++i) {
		...
		render::indicator.string(20, g_cl.m_height / 2 - (30 * i), indicator.color, indicator.text);
	}
}
...
void Visuals::PenetrationCrosshair() {
	...
	int pen_dmg = g_cl.m_pen_data.m_damage;

	if (g_cl.m_pen_data.m_pen || valid_player_hit)
		render::esp_small.string(x + 3, y + 2, { final_color }, std::to_string(pen_dmg).c_str(), render::ALIGN_LEFT);
...
void Visuals::DrawPlantedC4() {
	...
	int         final_damage;
	std::string time_str, damage_str, bombsite;
	Color       damage_color;
	vec2_t      screen_pos;
	...
	// 'on screen (2D)'.
	if (mode_2d) {
		std::string timer = tfm::format(XOR("%s - %.1fs"), m_last_bombsite.c_str(), explode_time_diff);
		std::string damage = tfm::format(XOR("%i"), final_damage);
		...
		if (m_c4_planted) {
			if (explode_time_diff > 0.f) {
				render::indicator.string(8, 11, { 0,0, 0, 125 }, timer);
				render::indicator.string(6, 10, colortimer, timer);
			}
			//Render.StringCustom(5, 0, 0, getSite(c4) + timer + "s", colortimer, font);
			if (g_cl.m_local->m_iHealth() <= final_damage) {
				render::indicator.string(8, 34, { 0,0, 0, 125 }, tfm::format(XOR("FATAL")));
				render::indicator.string(6, 33, { 192, 32, 17, 255 }, tfm::format(XOR("FATAL")));
			}
			else if (final_damage > 1) {
				render::indicator.string(8, 34, { 0,0, 0, 125 }, tfm::format(XOR("- %iHP"), damage));
				render::indicator.string(6, 33, { 255, 255, 152, 255 }, tfm::format(XOR("- %iHP"), damage));
			}
		}
	}
	...
	if (!g_cl.m_processing) {
		...
		for (size_t i{ impact_count }; i-- > 0; ) {
			...
			if (is_final_impact) {
				...
				beam_info.m_flWidth = 0.75f;
				beam_info.m_flEndWidth = 0.75f;
				beam_info.m_flFadeLength = 100.f;
...
void Visuals::AspectRatio() {
	static auto r_aspectratio = g_csgo.m_cvar->FindVar(HASH("r_aspectratio"));

	if (g_menu.main.visuals.aspect_ratio.get()) {
		r_aspectratio->SetValue(g_menu.main.visuals.aspect_ratio_amt.get() / 100);
	}
	else {
		r_aspectratio->SetValue(1.50f);
	}
}